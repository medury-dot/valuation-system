#!/usr/bin/env python3
"""
Round 3: Insert 842 missing companies into mssdb.kbapp_marketscrip and rag.vs_active_companies.

Steps:
1. Load expand_active_errors_round2.csv (842 unmatched companies)
...
# Paths
ERRORS_CSV = "/Users/ram/code/research/expand_active_errors_round2.csv"
EXCEL_FILE = "/Users/ram/code/research/valuation_group-valuation_subgroup-feb2026.xlsx"
LOG_FILE = "/Users/ram/code/research/expand_active_companies_round3.log"
ERRORS_OUT = "/Users/ram/code/research/expand_active_errors_round3.csv"
...
def main():
    ...
    try:
        # 1. Load errors CSV (842 companies)
        log("Loading errors CSV...", log_fh)
        errors_df = pd.read_csv(ERRORS_CSV, dtype=str)
        log(f"  Loaded {len(errors_df)} rows from errors CSV", log_fh)
        log(f"  Columns: {list(errors_df.columns)}", log_fh)

        error_accord_codes = set(errors_df["accord_code"].dropna().unique())
        log(f"  Unique accord_codes in errors CSV: {len(error_accord_codes)}", log_fh)
        ...
        log(f"  Excel columns: {list(excel_df.columns)}", log_fh)
        ...
        # 5. Check for existing accord_codes in marketscrip (equity only)
        ...
        for idx, row in errors_df.iterrows():
            ...
            # Get data - prefer Excel, fallback to errors CSV columns
            ...
            if excel_row is not None:
                ...
                # Errors CSV has these columns merged from prior Excel join
                company_name = clean_str(row.get("Company Name")) or clean_str(row.get("company_name"))
                nse_symbol = clean_str(row.get("CD_NSE Symbol1"))
                bse_code = clean_str(row.get("CD_Bse Scrip ID"))
                cd_sector = clean_str(row.get("CD_Sector"))
                cd_industry = clean_str(row.get("CD_Industry1"))
                val_group = clean_str(row.get("valuation_group"))
                val_subgroup = clean_str(row.get("valuation_subgroup"))
                log(f"  WARN {accord_code}: using errors CSV data (no Excel match) - {company_name}", log_fh)

            if not company_name:
                errors.append({
                    "accord_code": accord_code,
                    "company_name": "",
                    "error_reason": "No company name found in Excel or errors CSV",
                })
                skipped_no_data += 1
            ...
            if accord_code in existing_active_accord:
                ...
                batch_active.append((
                    ...
                    "WEEKLY", 5, 1,
                    "excel_expansion_round3_feb2026"
                ))
        ...
        for row in cursor.fetchall():
            log(f"  {row['valuation_group']}: {row['cnt']}", log_fh)
        ...
        cursor.execute(
            "SELECT COUNT(*) as cnt FROM rag.vs_active_companies WHERE added_by = 'excel_expansion_round3_feb2026'"
        )
        round3_active = cursor.fetchone()["cnt"]
        log(f"Round 3 vs_active entries: {round3_active}", log_fh)

        # Summary
        log("\n=== SUMMARY ===", log_fh)
        log(f"Input rows: {len(errors_df)}", log_fh)
